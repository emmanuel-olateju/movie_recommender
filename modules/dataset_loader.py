...
class MovieLensDataset_OptimizedCuPy:
    def __init__(self, CSV_DIR: str, MOVIES_DIR: str=None, use_gpu: bool=True) -> None:
        self.use_gpu = use_gpu
        self.xp = cp if use_gpu else np  # xp will be cupy or numpy
        ...
        # Data loading stays on CPU (file I/O)
        with open(CSV_DIR, "r", encoding="utf-8") as file:
            next(file)
        ...
        self.users = self.xp.array(self.users)
        self.movies = self.xp.array(self.movies)
        self.ratings = self.xp.array(self.ratings)
    ...
    def compute_loss(self, mode="train"):
        ...
        # All operations automatically run on GPU with CuPy
        pred_m_rated = self.mu + self.xp.sum(self.V[movies_idx] * self.U[users_idx], axis=1) + self.BM[users_idx] + self.BN[movies_idx]
        rating_errors = (ratings - pred_m_rated) ** 2
        total_ratings = len(rating_errors)
        ...
        rating_error = self.xp.sum(rating_errors)

        # RMSE calculation
        rmse = float(self.xp.sqrt(rating_error / total_ratings)) if total_ratings > 0 else 0.0

        # Loss differs by mode
        if mode == "train":
            users_norm = self.xp.sum(self.U ** 2)
            movies_norm = self.xp.sum(self.V ** 2)
            users_bias_squared = self.xp.sum(self.BM ** 2)
            movies_bias_squared = self.xp.sum(self.BN ** 2)
    ...
    def train(self, test_size=0.1, latent_dim=10, n_iter=50, eval_inter=5, lambda_=1, tau=1, gamma=1, verbose=True):
        ...
        # Initialize on GPU
        self.U = self.xp.random.randn(M, K)
        self.V = self.xp.random.randn(N, K)
        self.BM = self.xp.random.randn(M)
        self.BN = self.xp.random.randn(N)
        self.mu = float(self.xp.mean(self.ratings[self.train_idx]))
        ...
        # Pre-build lookups - keep as CPU lists for indexing
        print("Building lookup tables ...")
        
        # Convert to CPU for building lookups (faster for this operation)
        if self.use_gpu:
            train_users_cpu = cp.asnumpy(train_users)
            train_movies_cpu = cp.asnumpy(train_movies)
        else:
            train_users_cpu = train_users
            train_movies_cpu = train_movies
            
        user_to_indices = [[] for _ in range(M)] 
        movie_to_indices = [[] for _ in range(N)]

        for idx in tqdm(range(len(train_users_cpu)), desc="Building lookups"):
            user_to_indices[train_users_cpu[idx]].append(idx)
            movie_to_indices[train_movies_cpu[idx]].append(idx)

        # Convert to arrays (keep as numpy for indexing, will convert data as needed)
        user_to_indices = [np.array(v, dtype=np.int32) if len(v) > 0 else np.array([], dtype=np.int32)
                           for v in user_to_indices]
        movie_to_indices = [np.array(v, dtype=np.int32) if len(v) > 0 else np.array([], dtype=np.int32)
                            for v in movie_to_indices]
        ...
        for epoch in tqdm(range(n_iter), total=n_iter):
            ...
            for m in range(M):
                ...
                # Index with numpy indices, but data is on GPU
                movies_rated_by_m = train_movies[indices]
                ratings_by_m = train_ratings[indices]

                # Detrend on-the-fly (operations on GPU)
                detrended = ratings_by_m - self.mu - self.BM[m] - self.BN[movies_rated_by_m]

                V_rated = self.V[movies_rated_by_m]
                numerator = lambda_ * self.xp.sum(V_rated * detrended[:, self.xp.newaxis], axis=0)
                denominator = lambda_ * (V_rated.T @ V_rated) + (tau * self.xp.eye(K))
                self.U[m] = self.xp.linalg.solve(denominator, numerator)
            ...
            for n in range(N):
                ...
                U_rated = self.U[users_rating_movie]
                numerator = lambda_ * self.xp.sum(U_rated * detrended[:, self.xp.newaxis], axis=0)
                denominator = lambda_ * (U_rated.T @ U_rated) + (tau * self.xp.eye(K))
                self.V[n] = self.xp.linalg.solve(denominator, numerator)

            # Bias updates - compute predictions on-the-fly
            # User biases
            predictions = self.mu + self.xp.sum(self.U[train_users] * self.V[train_movies], axis=1) + self.BN[train_movies]
            residuals = train_ratings - predictions
            per_user_residual_sum = self.xp.bincount(train_users, weights=lambda_*residuals, minlength=M)
            per_user_count = self.xp.bincount(train_users, minlength=M)
            mask = per_user_count > 0
            self.BM[mask] = per_user_residual_sum[mask] / (lambda_ * per_user_count[mask] + gamma)

            # Movie biases (recompute predictions with updated BM)
            predictions = self.mu + self.xp.sum(self.U[train_users] * self.V[train_movies], axis=1) + self.BM[train_users]
            residuals = train_ratings - predictions
            per_movie_residual_sum = self.xp.bincount(train_movies, weights=lambda_*residuals, minlength=N)
            per_movie_count = self.xp.bincount(train_movies, minlength=N)
            mask = per_movie_count > 0
            self.BN[mask] = per_movie_residual_sum[mask] / (lambda_ * per_movie_count[mask] + gamma)
    ...
    def load_model(self, model_dir):
        ...
        # Load to GPU if using GPU
        self.U = self.xp.array(hyper_params['U'])
        self.V = self.xp.array(hyper_params['V'])
        self.K = self.U.shape[-1]
        
        self.BM = self.xp.array(hyper_params['BM'])
        self.BN = self.xp.array(hyper_params['BN'])

        self.mu = float(hyper_params['mu'])
        self.lambda_ = float(hyper_params['lambda_'])
        self.gamma = float(hyper_params['gamma'])
        self.tau = float(hyper_params['tau'])

    def save_model(self, save_path):
        """Save model parameters to disk"""
        # Convert GPU arrays back to CPU for saving
        if self.use_gpu:
            np.savez(save_path,
                    U=cp.asnumpy(self.U),
                    V=cp.asnumpy(self.V),
                    BM=cp.asnumpy(self.BM),
                    BN=cp.asnumpy(self.BN),
                    mu=self.mu,
                    lambda_=self.lambda_,
                    gamma=self.gamma,
                    tau=self.tau)
    ...
    def train_test_split(self, split_ratio: float):
        ...
        # This stays on CPU since it's just index generation
        for idx in tqdm(range(self.__n_entries), total=self.__n_entries):
            if random.uniform(0.0, 1.0) < split_ratio:
                train_idxs.append(idx)
        ...
        train_idxs = self.xp.array(train_idxs)
        val_idxs = self.xp.array(val_idxs)
        test_idxs = self.xp.array(test_idxs)
        return train_idxs, val_idxs, test_idxs
